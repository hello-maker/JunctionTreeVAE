...
class JTNNVAE(nn.Module):
    ...
    def assm(self, mol_batch, mol_vec, tree_mess):
        ...
        cand_vec = self.G_mean(cand_vec)  # shape : [num of candidate, 28]
        ...
        scores = torch.bmm(mol_vec, cand_vec).squeeze() #scores.shape : [num of candidate]
        ...
        for i,mol_tree in enumerate(mol_batch):
            ...
            for node in comp_nodes:
                ## node.cands = enum_assemble(self, neighbors)
                # >> 가능한 candidate를 모두 뽑아냄!
                ## label : candidate 중 answer label                
 ...
 #cur_scores.data[label] >= cur_scores.max().data[0]
        st,acc = 0,0
        all_loss = []
        for label,le in labels:
            cur_scores = scores.narrow(0, st, le)   
            #if cur_scores[label] >= cur_scores.max()[0]: 
    ...
    def decode(self, tree_vec, mol_vec, prob_decode):
        ...
        # global_amap은 atom의 id(ex. C or O or ...)를 key값으로
        # cur_mol : root_node


        # 여기가 subgraph를 combination 하는 과정
        ...
        # tree_mess : tree encoder의 root node
...
#cur_mol = self.dfs_assemble(tree_mess, mol_vec, pred_nodes, cur_mol, global_amap, [], pred_root, None, prob_decode)
        ...
        children = [nei for nei in cur_node.neighbors if nei.nid != fa_nid]
                                # cur_node : pred_root_node
        neighbors = [nei for nei in children if nei.mol.GetNumAtoms() > 1]
        neighbors = sorted(neighbors, key=lambda x:x.mol.GetNumAtoms(), reverse=True)
        singletons = [nei for nei in children if nei.mol.GetNumAtoms() == 1]
        neighbors = singletons + neighbors
        # root노드의 이웃 노드를 보고 원자(or bond)단위인지 링단위인지 구분
        
        cur_amap = [(fa_nid,a2,a1) for nid,a1,a2 in fa_amap if nid == cur_node.nid]
        cands = enum_assemble(cur_node, neighbors, prev_nodes, cur_amap)
        # enum_assemble에서 candidate(화학적으로 가능한 using RDKit)를 뽑아내는 과정
        ...
        scores = torch.mv(cand_vecs, mol_vec) * 20  # len of scores : num of candidate
