...
def enum_attach(ctr_mol, nei_node, amap, singletons):
    ...
    if nei_mol.GetNumBonds() == 0: #neighbor singleton  # 이웃노드가 원자일 때
        nei_atom = nei_mol.GetAtomWithIdx(0)
        used_list = [atom_idx for _,atom_idx,_ in amap]
        for atom in ctr_atoms:
            # atom : 현재 node의 모든 atom
    ...
    elif nei_mol.GetNumBonds() == 1: #neighbor is a bond  # 이웃노드가 bond 일 때
...
# parameter : cur_node, neighbors, prev_nodes, cur_amap    
    all_attach_confs = []
    ...
    def search(cur_amap, depth):
        ...
        # Atom일 때 : (nei_idx, atom.GetIdx(), 0)
        # Bond일 때 : (nei_idx, atom.GetIdx(), (b1 or b2).GetIdx())
        # Ring일 때 : (nei_idx, a1.GetIdx(), a2.GetIdx())
...
def dfs_assemble(cur_mol, global_amap, fa_amap, cur_node, fa_node):
    ...
    # 현재 노드의 이웃 노드(In Juction Tree)를 얻어내는 과정
